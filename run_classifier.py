...
class BinaryProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        return self._create_examples(self._read_tsv(os.path.join(data_dir, 'train', 'shf_train.csv')))

    def get_dev_examples(self, data_dir):
        return self._create_examples(self._read_tsv(os.path.join(data_dir, 'test', 'shf_test.csv')))

    def get_test_examples(self, data_dir):
        return self._create_examples(self._read_tsv(os.path.join(data_dir, 'train', 'shf_test.csv')))
    ...
    def _create_examples(self, texts):
        ...
        for i, line in enumerate(texts):
            examples.append(InputExample(guid=i, text_a=line[0], label=line[1]))
        return examples
...
class MulticlassProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        return self._create_examples(self._read_tsv(os.path.join(data_dir, 'train', '10news', 'cnews.train.csv')))
    ...
    def _create_examples(self, texts):
        examples = []
        for i, line in enumerate(texts):
            examples.append(InputExample(guid=i, text_a=line[1], label=line[0]))
        return examples
...
def file_based_convert_examples_to_features(examples, label_list, max_seq_length, tokenizer, output_file):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        feature, len_tokens = convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer)
    ...
    tf.logging.info("75%%的长度:%s" % np.percentile(length, 75))
...
def main(_):
    ...
    processors = {"binaryprocessor": BinaryProcessor, 'multiclassprocessor': MulticlassProcessor}
