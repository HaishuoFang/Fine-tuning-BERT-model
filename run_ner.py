...
flags.DEFINE_integer("max_seq_length", 25, "The maximum total input sequence length after WordPiece tokenization. "
...
class DataProcessor(object):
    ...
    def _read_txt(cls, input_file):
        with tf.gfile.Open(input_file, 'r') as f:
            context = f.readlines()
            lines = list(map(lambda x: x.strip(), context))
            return lines


class NERProcessor(DataProcessor):
    """Processor for the product data set (GLUE version).
        :return list[example1(id,text,label),example2]
    """

    def get_train_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_txt(os.path.join(data_dir, 'train', "texts.txt")),
                                     self._read_txt(os.path.join(data_dir, 'train', "labels.txt")), "train")

    def get_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_txt(os.path.join(data_dir, 'dev', "texts.txt")),
                                     self._read_txt(os.path.join(data_dir, 'dev', "labels.txt")), "dev")

    def get_test_examples(self, data_dir):
        """See base class."""
        return self._create_examples(self._read_txt(os.path.join(data_dir, 'test', "texts.txt")),
                                     self._read_txt(os.path.join(data_dir, 'test', "labels.txt")), "test")
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer):
    ...
    for i, token in enumerate(tokens_a):
        tokens.append(token)
        labels_id.append(label_map[example.label[i]])
        segment_ids.append(0)
    tokens.append("[SEP]")
    labels_id.append(0)
    segment_ids.append(0)
    ...
    if ex_index < 5:
        ...
        tf.logging.info("label: %s (id = %s)" % (example.label, labels_id))

    feature = InputFeatures(input_ids=input_ids, input_mask=input_mask, segment_ids=segment_ids, label_id=labels_id)
    return feature
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids, labels, num_labels,
    ...
    # instead.
    output_layer = model.get_sequence_output(bert_config)
    print(output_layer)

    hidden_size = output_layer.shape[-1].value

    output_weights = tf.get_variable("output_weights", [hidden_size, num_labels],
    ...
    with tf.variable_scope("loss"):
        ...
        logits = tf.tensordot(output_layer, output_weights, axes=[[-1], [0]])
        # logits:[batch,seq_len,num_classes]
        logits = tf.nn.bias_add(logits, output_bias)
        probabilities = tf.nn.softmax(logits, axis=-1)
        log_probs = tf.nn.log_softmax(logits, axis=-1)
        labels = labels[:, 1:]
        # one_hot_labels [batch,seq_len,num_classes]
...
def main(_):
    ...
    run_config = tf.contrib.tpu.RunConfig(cluster=tpu_cluster_resolver, master=FLAGS.master, model_dir=FLAGS.output_dir,
    ...
    run_config = tf.estimator.RunConfig(session_config=session_config, model_dir=FLAGS.output_dir,
    ...
    model_fn = model_fn_builder(bert_config=bert_config, num_labels=len(label_list),
                                ...
                                use_tpu=FLAGS.use_tpu, use_one_hot_embeddings=FLAGS.use_tpu)
    ...
    if FLAGS.do_predict:
        ...
        predict_input_fn = file_based_input_fn_builder(input_file=predict_file, seq_length=FLAGS.max_seq_length,
                                                       is_training=False, drop_remainder=predict_drop_remainder)

        result = estimator.predict(input_fn=predict_input_fn)

        id_label = {i: label for (i, label) in enumerate(label_list)}
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for prediction in result:
                ...
                for id in prediction:
                    label.append(id_label[id])
                output_line = "\t".join(id_label[id] for id in prediction) + "\n"
