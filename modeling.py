...
class BertModel(object):
  ...
  """
    ...
    def __init__(self, config, is_training, input_ids, input_mask=None, token_type_ids=None,
                 use_one_hot_embeddings=True, scope=None):
    ...
    """
        ...
        with tf.variable_scope(scope, default_name="bert"):
            ...
            with tf.variable_scope('sequence_output'):
                every_token_tensor = self.sequence_output[:, 1:, :]
                every_token_tensor = tf.reshape(every_token_tensor, (-1, self.sequence_output.get_shape()[-1]))

                self.seq_dense_output = tf.layers.dense(every_token_tensor, config.hidden_size, activation=tf.tanh,
                                                        kernel_initializer=create_initializer(config.initializer_range))
                self.seq_dense_output = tf.reshape(self.seq_dense_output, (batch_size, -1, config.hidden_size))
...
def embedding_postprocessor(input_tensor, use_token_type=False, token_type_ids=None, token_type_vocab_size=16,
  ...
  """
    ...
    if use_token_type:
        ...
        token_type_table = tf.get_variable(name=token_type_embedding_name, shape=[token_type_vocab_size, width],
                                           initializer=create_initializer(initializer_range))
...
def attention_layer(from_tensor, to_tensor, attention_mask=None, num_attention_heads=1, size_per_head=512,
                    query_act=None, key_act=None, value_act=None, attention_probs_dropout_prob=0.0,
                    initializer_range=0.02, do_return_2d_tensor=False, batch_size=None, from_seq_length=None,
                    to_seq_length=None):
        ...
        with tf.variable_scope("layer_%d" % layer_idx):
            ...
            with tf.variable_scope("attention"):
                ...
                with tf.variable_scope("self"):
                    attention_head = attention_layer(from_tensor=layer_input, to_tensor=layer_input,
                                                     attention_mask=attention_mask,
                                                     num_attention_heads=num_attention_heads,
                                                     size_per_head=attention_head_size,
                                                     attention_probs_dropout_prob=attention_probs_dropout_prob,
                                                     initializer_range=initializer_range, do_return_2d_tensor=True,
                                                     batch_size=batch_size, from_seq_length=seq_length,
